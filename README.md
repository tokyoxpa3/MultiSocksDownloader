...
- **串流解析下載（YouTube 等影音網站）**：勾選「解析影片連結」後，用 yt-dlp 把影片/音訊網頁網址解析成單檔直連 URL，再交給現有的多代理分段引擎續傳；DASH/HLS 分段串流改走 yt-dlp 原生下載，並以內建 ffmpeg 合併視訊+音訊為 MP4，可選畫質（最高 4K）與 FPS 上限。DRM 加密串流會自動略過，不會試圖破解。
